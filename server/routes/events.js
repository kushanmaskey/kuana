...
router.get('/:id', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query('SELECT * FROM events WHERE id = $1 AND is_published = true', [req.params.id]);
...
router.patch('/:id/featured', requireAuth, async (req, res) => {
  ...
  try {
    if (is_featured) {
      await pool.query('UPDATE events SET is_featured = false, updated_at = NOW() WHERE is_featured = true AND id <> $1', [req.params.id]);
    }
